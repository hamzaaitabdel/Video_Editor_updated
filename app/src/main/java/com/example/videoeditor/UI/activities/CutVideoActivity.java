...
public class CutVideoActivity extends AppCompatActivity implements View.OnClickListener {
...
//    private TextView txtVideoCancel;
    ...
    private AdUtils.Inters mInterstitialAd2;
    private void initAds() {
        //ads
        FrameLayout adContainer = findViewById(R.id.adView_cut);
        loadBanner(this,adContainer);
        mInterstitialAd2 = new AdUtils.Inters(this,false);

    }

    OnVideoTrimListener mOnVideoTrimListener;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        initAds();
        mOnVideoTrimListener = new OnVideoTrimListener() {
            @Override
            public void onTrimStarted() {
                // Create an indeterminate progress dialog

                mProgressDialog = new ProgressDialog(CutVideoActivity.this);
            ...
            public void getResult(Uri uri) {
                ...
                Intent aintnt = new Intent(CutVideoActivity.this,VideoActivity.class);
                aintnt.putExtra("path",uri.toString());
                startActivity(aintnt);
                finish();
            }
...
//        txtVideoEditTitle = (TextView) findViewById(R.id.txtVideoEditTitle);
        ...
        if (getIntent().getExtras() != null) {
            srcFile = getIntent().getExtras().getString("uri");
        }
        dstFile = Environment.getExternalStorageDirectory() + "/" + getString(R.string.app_name) +"/"+
                "/Videos/"+
                getString(R.string.app_name)+new Date().getTime()
    ...
    public void onClick(View view) {
        if (view == txtVideoUpload) {
            int diff = mEndPosition - mStartPosition;
            if (diff < 3) {
                Toast.makeText(CutVideoActivity.this, getString(R.string.video_length_validation),
                        Toast.LENGTH_LONG).show();
            } else {
                MediaMetadataRetriever
                        mediaMetadataRetriever = new MediaMetadataRetriever();
                mediaMetadataRetriever.setDataSource(CutVideoActivity.this, Uri.parse(srcFile));
